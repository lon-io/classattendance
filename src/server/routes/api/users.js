...
// find by id
router.get('/student_by_uid/:id', function(req, res) {
    Students.findOne({rfid_uid: req.params.id})
        .lean()
        .exec()
        .then(doc => {
            if(doc !== null) {
                ...
                return res.json({error: true, message: "No such User"})
            }
        })
        .catch(err => {
            return console.error(err);
...
router.get('/lecturer/:id', function(req, res) {
    Lecturers.findOne({_id: req.params.id})
        ...
        .exec()
        .then(doc => {
            Courses.find({coordinator: doc._id})
                .exec()
                .then(courses => {
                    doc.courses = courses;
                    return res.json(doc)
